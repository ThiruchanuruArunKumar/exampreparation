...
async function ensureSeedData(sb: any, force = false) {
  try {
    ...
    const { data: existingQuestions } = await sb.from("ipe_questions").select("id").limit(1);
...
export const getIpeSubjectsAndChapters = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data: subjects, error: subErr } = await sb
      ...
      .order("name");
    if (subErr) {
      if (subErr.code === "42P01") return { subjects: [], chapters: [] };
      throw subErr;
    }
    ...
    const { data: chapters, error: chapErr } = await sb
      ...
      .order("chapter_order");
    if (chapErr && chapErr.code !== "42P01") throw chapErr;
